...
class WPA_WCPB_Frontend {
	...
	public static function init() {
		...
		add_action( 'woocommerce_add_order_item_meta', array( __CLASS__, 'wpa_wcpb_woocommerce_change_checkout_order_detail' ), 1, 2 );
	...
	public static function wpa_wcpb_add_to_cart() {
		...
		if ( count( $arr_product_id ) > 0 && is_numeric( $arr_product_id[0] ) ) {
			...
			if ( $product_parent ) {
				...
				self::wpa_wcpb_add_to_cart_total_discount( $arr_product_id, $product_bundle, $main_pro_variable, $main_pro_meta, $custom_bundle_variable );
			}
		}
		die;
	}
	 ...
	 * @return 
	 */
	public static function wpa_wcpb_add_to_cart_total_discount( $arr_product_id, $product_bundle, $variation_id, $product_variable, $custom_bundle_variable ) {
		...
		if ( ! empty( $arr_product_id[0] ) ) {
			$main_product = wc_get_product( $arr_product_id[0] );
			if ( $main_product && $main_product->is_in_stock() ) {
                ...
                if ( $main_product->is_type( 'variable' ) && !empty($product_variable) ) {
                    //$main_variation = new WC_Product_Variable( $main_product_id );
                    //$main_product_custom_price = $main_variation->get_price();

                    $variable_main_product = wp_unslash( $product_variable );

                    if ( version_compare( WC_VERSION, '3.0.0', '<' ) ) {
                        $variation_id = $main_product->get_matching_variation( $variable_main_product );
                    } else {
                        $data_store   = WC_Data_Store::load( 'product' );
                        $variation_id = $data_store->find_matching_product_variation( $main_product, $variable_main_product );
                    }
                    $main_product_available_variation = $main_product->get_available_variation( $variation_id );

                    $main_product_custom_price = $main_product_available_variation['display_price'];
	 ...
	 * @return 
	 */
	public static function wpa_wcpb_woocommerce_change_checkout_order_detail( $item_id, $values ) {
		...
		if ( $bundles && count( $bundles_added ) > 1 ) {
			$order_extra_meta_data .= '<ul style="clear: both;">';
			...
			$order_extra_meta_data .= '</ul>';
			wc_add_order_item_meta( $item_id, __( 'Product Bundles', 'wcpb' ), $order_extra_meta_data );
		}
	}

function wpa_wcpb_add_to_cart( obj ) {
    ...
    if(jQuery(loader).hasClass('done')){
        jQuery(loader).removeClass('done');
    }

	jQuery( '.list-selects .list-item').each(function(){
		var checked = jQuery( 'input[type="checkbox"]:checked', jQuery(this) ).length;
		if ( checked ) {
			list_product_id += jQuery(this).find('.item-select').attr('data-product-id') + ',';
		}
	});
...
function wpa_wcpb_toggleClass_loading(toggleClassName, target) {
    ...
    if(jQuery(target).hasClass('done')){
        jQuery(target).removeClass('done');
    }else{
        jQuery(target).addClass('done')
    }
...
function wpa_wcpb_onchange_input_check_total_discount(){
	...
	jQuery('.px-product-bundles .list-select input[type=checkbox]:checked').each(function(){
		...
		jQuery('.price > span.woocommerce-Price-amount', parent).html(
			new_price.toFixed(0) + '<span class="woocommerce-Price-currencySymbol">' + jQuery('.price > span.woocommerce-Price-amount .woocommerce-Price-currencySymbol', parent).html() + '</span>'
		);
		total_price += parseFloat( price );
	});

	jQuery('.total.price .current-price span.woocommerce-Price-amount', wpa_wcpb_list).html(
		(parseFloat( total_price ) - parseFloat( total_price ) * parseFloat( bundle_percent ) / 100 ).toFixed(0) + currencySymbol
	);
	jQuery('.total.price .old-price span.woocommerce-Price-amount', wpa_wcpb_list).html(
		total_price.toFixed(0) + currencySymbol
	);
	jQuery('.total.price .save-price span.woocommerce-Price-amount', wpa_wcpb_list).html(
		Math.round( parseFloat( total_price ) - ( parseFloat( total_price ) - parseFloat( total_price ) * parseFloat( bundle_percent ) / 100 ) ).toFixed(0) + currencySymbol
    );
		...
		$( '.plt-variations-form select' ).each(function() {
			$(this).change(function(){
				...
				if (bundle_price == false) {
					...
					$('.info-item .price del .woocommerce-Price-amount', parent).html( Math.round( parseFloat( bundle_price ) ).toFixed(0) + currencySymbol);
